...
parser.add_argument('--output-csv', required=True, help='Directory to save prediction CSVs')
parser.add_argument('--output-plots', required=True, help='Directory to save plots')
parser.add_argument('--output-model', required=True, help='Directory to save trained model')
...
df_predictions_no_target[df_predictions_no_target['predicted'] == 'cancer']\
    .to_csv(os.path.join(args.output_csv, 'Cancer_Patient_Predictions_OUT.csv'), index=False)
df_predictions_no_target[df_predictions_no_target['predicted'] == 'no cancer']\
    .to_csv(os.path.join(args.output_csv, 'Non_Cancer_Patient_Predictions_OUT.csv'), index=False)

# -------------------------------
# Save model
# -------------------------------
model.save_model(os.path.join(args.output_model, 'Breast_Cancer_Prediction_Model.json'))
...
bars = sns.barplot(x=pred_counts.index, y=pred_counts.values, palette="Set2")
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=["cancer","no cancer"], yticklabels=["cancer","no cancer"])
