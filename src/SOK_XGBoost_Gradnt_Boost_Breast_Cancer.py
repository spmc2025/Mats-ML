...
# Save output CSVs
df_predictions_no_target.to_csv("output/All_Patient_Predictions_OUT.csv", index=False)
cancer_patients_df = df_predictions_no_target[df_predictions_no_target['predicted'] == 'cancer']
cancer_patients_df.to_csv("output/Cancer_Patient_Predictions_OUT.csv", index=False)
non_cancer_patients_df = df_predictions_no_target[df_predictions_no_target['predicted'] == 'no cancer']
non_cancer_patients_df.to_csv("output/Non_Cancer_Patient_Predictions_OUT.csv", index=False)
...
# Save model
model.save_model("output/Breast_Cancer_Prediction_Model.json")

# Feature importance plot
xgb.plot_importance(model, max_num_features=10, height=0.5, importance_type='weight')
plt.title("Top 10 Important Features")
plt.tight_layout()
plt.savefig("output/Breast_Cancer_Feature_Importance_OUT.png")
plt.close()
...
plt.tight_layout()
plt.savefig("output/Breast_Cancer_Barchart.png")
plt.close()
...
plt.ylabel("Actual")
plt.tight_layout()
plt.savefig("output/Breast_Cancer_CM_OUT.png")
plt.close()
# azure-pipelines.yml
"""
